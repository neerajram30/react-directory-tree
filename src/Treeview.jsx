...
function Treeview(props) {
  const { width, getSelectedNode, data } = props;
          ...
          nodeItem.type === 'folder' ?
            <div>
              <button onClick={(e) => handleCollapse(e, nodeItem)} className='text-md hover:bg-gray-100 w-1/2 text-start pl-2 pr-2 flex items-center justify-start'>
                <span className='pt-[1px]'>{collapsed?.find((item) => item?.id === nodeItem?.id)?.id === nodeItem?.id ? 
                  <ChevronDownIcon className='size-6 text-gray-300' /> : <ChevronRightIcon className="size-6 text-gray-300" />}</span>
